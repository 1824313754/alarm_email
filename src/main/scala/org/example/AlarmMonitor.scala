...
object AlarmMonitor {
  def main(args: Array[String]): Unit = {
    ...
    while (true) {
      ...
      if (hour >= start && hour <= end) {
        ...
        val connection2 = new ClickhouseUtils().getConnection2
        val resultSet: ResultSet = connection.createStatement().executeQuery(sql)
        val resultSet2 = connection2.createStatement().executeQuery(sql2)
        //将resultSet2和resultSet合并到
        val alarmDataList: immutable.Seq[AlarmData] = extractData(resultSet)
        val alarmDataList2: immutable.Seq[AlarmData] = extractData(resultSet2)
        // 合并两个 List
        val mergedList= alarmDataList ++ alarmDataList2
        //定义一个集合存放结果集
        val list = scala.collection.mutable.ListBuffer[String]()
        // 检查每条记录的时间差值
        mergedList.foreach {
          data =>
            val vehicleFactory = data.getVehicleFactory
            val ctime = data.getCtime
            val source_type = data.getSource_type
            val currentTime = data.getCurrentTime
            val alarmData = new AlarmData(vehicleFactory, ctime, source_type, currentTime)
            ...
            if (source_type != "alarm" && minutesDiff > delayTime) {
              ...
              if (vehicleFactoryName != null) {
                //获取当前车厂号对应的延迟时间
                val alarmDelayTimeValue = Option(alarmDelayTime.getString(vehicleFactoryName)).getOrElse("25").toInt
  ...
  def extractData(resultSet: ResultSet): List[AlarmData] = {
    var list: List[AlarmData] = List()
